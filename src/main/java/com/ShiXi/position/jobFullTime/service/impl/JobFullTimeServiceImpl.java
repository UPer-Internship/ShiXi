package com.ShiXi.position.jobFullTime.service.impl;
...
import com.ShiXi.position.jobFullTime.domin.vo.JobFullTimeVO;
import com.ShiXi.position.jobFullTime.entity.JobFullTime;
import com.ShiXi.common.mapper.JobFullTimeMapper;
...
@Slf4j
@Service
public class JobFullTimeServiceImpl extends ServiceImpl<JobFullTimeMapper, JobFullTime> implements JobFullTimeService {
    
    @Override
    public Result createJobFullTime(JobFullTimeCreateDTO createDTO) {
        try {
            // 参数校验
            if (createDTO == null) {
            ...
            BeanUtils.copyProperties(createDTO, jobFullTime);
            jobFullTime.setPublisherId(publisherId);
            jobFullTime.setStatus(1); // 默认可见
            
            // 保存到数据库
            boolean success = this.save(jobFullTime);
            
            if (success) {
                // 转换为VO返回
                JobFullTimeVO vo = new JobFullTimeVO();
                BeanUtils.copyProperties(jobFullTime, vo);
                return Result.ok(vo);
            } else {
                return Result.fail("创建岗位失败");
            }
        } catch (Exception e) {
            log.error("创建全职岗位失败", e);
            return Result.fail("系统繁忙，请稍后重试");
        }
    }
    
    @Override
    public Result updateJobFullTime(JobFullTimeUpdateDTO updateDTO) {
        try {
            // 参数校验
            if (updateDTO == null) {
                return Result.fail("请求参数不能为空");
            ...
            // 检查岗位是否存在
            JobFullTime existingJob = this.getById(updateDTO.getId());
            if (existingJob == null) {
            ...
            if (StringUtils.hasText(updateDTO.getSalaryRound())) {
                existingJob.setSalaryRound(updateDTO.getSalaryRound());
            }
            ...
            log.error("更新全职岗位失败", e);
            return Result.fail("系统繁忙，请稍后重试");
        }
    }
    
    @Override
    public Result deleteJobFullTime(Long id) {
        try {
            // 参数校验
            if (id == null || id <= 0) {
                return Result.fail("岗位ID不能为空且必须为正数");
            ...
            // 检查岗位是否存在
            JobFullTime existingJob = this.getById(id);
            if (existingJob == null) {
                return Result.fail("岗位不存在");
            }
